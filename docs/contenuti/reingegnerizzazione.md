...
15. possibilità per il personale della PA (in particolare per il responsabile del sistema di gestione documentale) di poter modificare|implementare la classe dei metadati dell'applicativo che gestisce il procedimento. Per questa operazione il personale della PA deve poter operare in piena autonomia (attraverso l'uso di apposite credenziali) senza fare ricorso alla software house che ha generato il software.
